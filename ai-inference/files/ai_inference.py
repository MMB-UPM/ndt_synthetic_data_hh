...
class AIInference:
    ...
    def __send_data(self, probs: List[dict], metadata: Any, version: str) -> None:
        """
            ...
            try:
                ...
                if messages:
                    ...
                    for message in messages:
                        ...
                        start = time.time()
                        probs = self.__predict(self.model, features, version)
                        LOGGER.info("Tiempo tardado en hace predict: %s", time.time()-start)
                        
                        if probs is not None:
                            self.__send_data(probs, metadata, version)
